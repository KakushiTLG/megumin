    ...
    if int(timesleep) < math.ceil(time.time()): 
        ...
        try:
            ...
            with db.cursor() as cursor:
                ...
                cursor.execute(sql, (minAtkMob, maxAtkMob, minHpMob, maxHpMob, int(location)))
        ...
        if m.reply_to_message: #если сообщение ответом то
            ...
            if str(player2) == str('710957371'):
                ...
                with db.cursor() as cursor:
                    ...
                    enemyMob = m.reply_to_message.from_user.username # имя моба.
                ...
                if enemyLvl >= 4:
                    db = pymysql.connect(host='localhost',
                                 ...
                                 cursorclass=pymysql.cursors.DictCursor)
                    with db.cursor() as cursor:
                        sql = "UPDATE `users` SET `points` = points - 2 WHERE user_id = %s"
...
#                    critStr = str("За бой ты смог нанести критический удар " + str(critStack) + " раз. Но это не помогло одержать верх в битве...\n ")
                ...
                timersearch = random.randint(2, 6)
                t = Timer(timersearch, battleResult, [m, searchMob, critStrEnemy, pFA, critStr, randomFirstAttack, eMSA, pSA, enemyMob, eventTitle, battleTitle, randomGojiraSpawn, uC, uFN, winner, playerHealth, playerAttack, enemyMobRandomAttack, enemyMobRandomHealth, playerStackHp, enemyMobStackHp])
                t.start()
...
def battleResult(m, searchMob, critStrEnemy, pFA, critStr, randomFirstAttack, eMSA, pSA, enemyMob, eventTitle, battleTitle, randomGojiraSpawn, uC, uFN, winner, playerHealth, playerAttack, enemyMobRandomAttack, enemyMobRandomHealth, playerStackHp, enemyMobStackHp):
    global timesleep
    global pvpBattle
